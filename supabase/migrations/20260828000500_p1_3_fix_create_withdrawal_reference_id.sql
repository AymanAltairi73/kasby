-- =============================================================================
-- P1-3 WITHDRAWAL BLOCKER FIX (approved scope)
--   Fix: transactions.agent_id  ->  transactions.reference_id  (p_agent_id::TEXT)
--
-- The live create_withdrawal was overwritten out-of-band and INSERTs into a
-- transactions.agent_id column that does NOT exist in the live schema (confirmed
-- read-only via information_schema). It also never stored the agent in
-- transactions.reference_id, so fn_process_agent_commission could not resolve the
-- agent (it reads reference_id).
--
-- This migration reproduces the EXACT live function body and changes ONLY:
--   1. The transaction INSERT column list: agent_id -> reference_id
--   2. The corresponding value:            p_agent_id -> p_agent_id::TEXT
--
-- Preserved verbatim from live:
--   * signature public.create_withdrawal(NUMERIC, UUID, TEXT, TEXT)
--   * RETURNS jsonb
--   * SECURITY DEFINER
--   * auth check, min amount, idempotency, ensure_user_wallet + FOR UPDATE lock,
--     frozen check, fee helper call (v_fee computed, NOT applied - out of scope),
--     available_balance/pending_balance movement, agent_transactions insert,
--     single user notification, EXCEPTION handler, return JSONB structure.
--
-- NOT changed: fee behavior, withdrawal amounts, wallet available/pending logic,
-- idempotency, agent_transactions, notifications, fn_process_agent_commission,
-- any other function/table/schema. No agent_id column added.
-- =============================================================================
...
BEGIN
  ...
  -- Record pending withdrawal transaction (FIX: reference_id stores p_agent_id::TEXT)
