...
class _SocialNetworkViewState extends State<SocialNetworkView>
        ...
        bottom: TabBar(
          ...
          tabs: [
            ...
            Obx(
              () => Tab(
                child: _badgedTab(
                  'incoming_requests'.tr,
                  _controller.incomingRequests.length,
                ),
              ),
            ),
            ...
            Obx(
              () => Tab(
                child: _badgedTab(
                  'outgoing_requests'.tr,
                  _controller.outgoingRequests.length,
                ),
              ),
            ),
      ...
      if (_controller.isLoading.value && _controller.suggestions.isEmpty) {
        return ListView.builder(
          ...
          itemBuilder: (_, __) => const KasbyShimmer.listItem(
            margin: EdgeInsets.only(bottom: KasbySpacing.sm),
          ),
        );
      }
          ...
          children: [
            Obx(
              () => KeyedSubtree(
                ...
                child: SocialDashboardHeader(
                  stats: _controller.dashboardStats.value,
                ),
              ),
            ),
  ...
  Widget _buildOutgoingTab() {
    return RefreshIndicator(
      ...
      child: Obx(() {
        if (_controller.isLoading.value &&
            _controller.outgoingRequests.isEmpty) {
          return _tabShimmer();
        }
        ...
        if (_controller.outgoingRequests.isEmpty) {
          return ListView(
            ...
            children: [
              _emptyState('no_outgoing_requests'.tr, Icons.outbox_rounded),
            ],
          );
        }
            ...
            child: Obx(() {
              ...
              return ListView.separated(
                ...
                itemBuilder: (_, i) {
                  ...
                  if (i == 0) {
                    return KeyedSubtree(
                      key: TourTargetKeys.socialFriends,
                      child: card,
                    );
                  }
